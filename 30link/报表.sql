/*每日开户数*/
select user_id,to_char(open_account_time,'yyyymmdd'),partner_id
from tb_silver_user_stat@silver_std
where to_char(open_account_time,'yyyymmdd')
between 20170501 and to_char(sysdate-1,'yyyymmdd')
order by to_char(open_account_time,'yyyymmdd')
...
FROM
  (
    SELECT
      ...
      b.user_name                              用户姓名,
      b.submit_time                            流转时间,
      round(nvl(b.net_zcmoney, 0)/10000 + nvl(b.net_in, 0)/10000,2) 激活资金
    ...
    SELECT
      ...
      c.user_name                        用户姓名,
      b.submit_time                      流转时间,
      round(HHT_NET_VALUE_SUB/10000 + HHT_NET_IN_SUB/10000,2) 激活资金
    FROM silver_consult.tb_crm_ia@consul_std a
