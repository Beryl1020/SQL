...
max(f.create_time)
from silver_consult.v_tb_crm_user@consul_std a
  left join
  silver_consult.tb_crm_ia@consul_std b
  on a.ia_id=b.id
  ...
  left join silver_njs.history_transfer@silver_std e
  on c.firm_id=e.firmid
  left join silver_consult.tb_crm_tel_record@consul_std f
...
from
  (select ia_id as ia_id, user_id as user_id,
        sum(worksec) as worksec,
        max(connect_time) as connect_time1, min(connect_time) as connect_time2
     from silver_consult.tb_crm_tel_record@consul_std
     group by ia_id,user_id) a

left join silver_consult.tb_crm_ia@consul_std b
  on a.ia_id=b.id
  left join silver_consult.v_tb_crm_user@consul_std c
  on a.user_id=c.id
  left join tb_silver_user_stat@silver_std d
  on c.fa_id=d.user_id
  left join silver_njs.history_transfer@silver_std e
  on d.firm_id=e.firmid
where b.group_id=103
      and a.user_id<>-1
...
select * from silver_consult.tb_crm_ia@consul_std where name='董学雷'
select * from silver_consult.v_tb_crm_user@consul_std
