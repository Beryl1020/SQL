...
from silver_consult.tb_crm_dispatch_his@consul_std a
  join silver_consult.tb_crm_tel_record@consul_std c
...
select * from silver_consult.tb_crm_dispatch_his@consul_std a                --个别用户检验
--join silver_consult.tb_crm_ia@consul_std b
--on a.ia_id=b.id
where a.user_id =1000513962


select id,name,group_id from silver_consult.tb_crm_ia@consul_std
...
join silver_consult.tb_crm_ia@consul_std ia on dis.ia_id=ia.id
  ...
  join silver_consult.tb_crm_transfer_record@consul_std d on dis.user_id=d.user_id

where ia.group_id in (2,3,4,5,6,9,10,11,12,105,112,113,106,114)
group by ia.group_id


select count(distinct a.user_id),b.group_id from silver_consult.tb_crm_transfer_record@consul_std a --Q1各组开单总量
  join silver_consult.tb_crm_ia@consul_std b
  on a.fia_id=b.id
...
from
  ...
  on a.user_id = b.id
  join tb_silver_user_stat@silver_std c  on b.fa_id=c.user_id
...
select distinct a.user_id,c.open_account_time from silver_consult.tb_crm_dispatch_his@consul_std a
  join silver_consult.v_tb_crm_user@consul_std b
  on a.user_id = b.id
  join tb_silver_user_stat@silver_std c  on b.fa_id=c.user_id
where a.type=24
