...
FROM
  ...
  (SELECT
     ...
     bbb.group_id,
     bbb.ia_name,
     round(nvl(aaa.当日达标用户数, 0) / bbb.当日流转用户数, 4) AS 当日流转当日达标率
   FROM
     (SELECT
        a.ia_id,
        a.group_id,
  ...
  (SELECT
     ...
     bbb.group_id,
     bbb.ia_name,
     round(nvl(aaa.当日流转开仓用户数, 0) / bbb.当日流转用户数, 4) AS 当日流转当日开仓率
   FROM
     (SELECT
        a.ia_id,
        a.group_id,
  ...
  (SELECT
   ...
   WHERE A.fdate BETWEEN to_char(trunc(sysdate - 1, 'd') + 1, 'yyyymmdd') AND to_char(sysdate - 1, 'yyyymmdd') AND
         A.realdate > b.submit_time
   GROUP BY c.ia_name, c.ia_id, c.group_id
  ) a7
  ...
  left join
     ...
     bbb.group_id,
     bbb.ia_name,
     round(nvl(aaa.当周达标用户数, 0) / bbb.当周流转用户数, 4) AS 本周截至当日达标率
   FROM
     (SELECT
        a.ia_id,
        a.group_id,
...
FROM
  ...
  left JOIN
    (SELECT
     bbb.group_id,
     round(sum(nvl(aaa.当周达标用户数, 0)) / sum(nvl(bbb.当周流转用户数,0)), 4) AS 当周截至当日达标率
   FROM
     (SELECT
        a.ia_id,
        a.group_id,
