 ...
 from
   (SELECT
    ...
    FROM
      ...
      ) a LEFT JOIN info_silver.DW_CRM_TRANS_DETAIL_BF b
        ON a.user_id = b.user_id AND b.stat_date = trunc(sysdate - 1)
   )a left join silver_njs.history_transfer@silver_std b
...
group by id,crm_name,user_id,fname,submit_time,dispatch_time,jhzj,firm_id,contnum)a left join
 info_silver.DW_CRM_TRANS_DETAIL_BF b
 on a.user_id=b.user_id and trunc(a.流转时间)=b.stat_date)a
 left join info_silver.tb_crm_transfer_record_his b
...
select * from info_silver.ods_crm_transfer_record where process <>-1

select * from info_silver.DW_CRM_TRANS_DETAIL_BF

select * from info_silver.tb_crm_transfer_record_his
...
select   * from DW_CRM_TRANS_DETAIL_BF@silver_stat_urs_30_link a  where a.stat_date = trunc(sysdate - 1)
...
select * from silver_njs.history_transfer@silver_std where  firmid = 163000000409344