...
from
  ...
  left join
  info_silver.ods_crm_transfer_record trans
  on aaa.id=trans.firm_id
  left JOIN
  silver_njs.history_transfer@silver_std inout
  on trans.firm_id=inout.firmid

  left JOIN
  silver_njs.tb_silver_data_center@silver_std asset
  on trans.firm_id=asset.firmid
  and hdate=20170314
  left JOIN
  info_silver.ods_history_user user30
  on trans.firm_id=user30.firm_id
  left JOIN
  tb_silver_user_stat@silver_std user1
