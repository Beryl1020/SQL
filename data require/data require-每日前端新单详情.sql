  ...
  (select *
   from
    (select distinct fa_id, b.user_name, a.fia_id, c.name, c.group_id, submit_time, net_assets
     ...
     left join
       (select distinct fa_id, id, user_name from silver_consult.v_tb_crm_user@consult_std) b
     on a.user_id = b.id
     left join
       (select distinct id, name, group_id from silver_consult.tb_crm_ia@consult_std) c
     on a.fia_id = c.id) t1
   left join
     (select user_id, firm_id from silver.tb_silver_user_stat
